...
def add_paragraph_runs(p, text, is_italic=False, default_color=None):
    ...
    # Standard regex to find bold and italic markdown markers
    ...
    while i < len(lines):
        ...
        if line.startswith('![') and ']' in line and '(' in line:
            ...
            if img_match:
                ...
                # If path has file:/// prefix, extract local path
                if img_path.startswith('file:///'):
                    img_path = img_path.replace('file:///', '').replace('/', os.sep)
                
                # Check if file exists, if not check relative path
                if not os.path.exists(img_path):
                    # check in workspace directory
                    alt_path = os.path.join(os.path.dirname(md_path), img_path)
        ...
        if line.startswith('|'):
            ...
            if len(table_lines) >= 3: # Need header, separator, and data
                # Extract header
                ...
                for ri, row_cells in enumerate(data_rows):
                    ...
                    for ci, val in enumerate(row_cells):
                        ...
                        # Center align numbers or short tags if needed, otherwise left
        ...
        if line:
            # Check if it's the author section right after title
            is_author = any(x in line for x in ["Tác giả:", "Đơn vị:", "Email:", "GitHub:", "Author:", "Affiliation:"])
            p = doc.add_paragraph()
            p.paragraph_format.space_after = Pt(6)
...
if __name__ == "__main__":
    ...
    convert_md_to_docx(
        os.path.join(artifact_dir, "IEEE_Research_RQ1.md"),
        os.path.join(workspace, "IEEE_Research_Paper_VI.docx")
    )
    
    # Generate English version
    convert_md_to_docx(
        os.path.join(artifact_dir, "IEEE_Research_RQ1_EN.md"),
        os.path.join(workspace, "IEEE_Research_Paper_EN.docx")
    )
